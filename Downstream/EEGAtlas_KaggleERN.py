...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        if x.shape[1] != self.chans_num:
            raise ValueError(f"Expected {self.chans_num} KaggleERN EEG channels, got {x.shape[1]}")
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        for key, value in results.items():
            if key == "balanced_accuracy":
                print(f"Balanced Accuracy: {value}")
...
for seed in [7,718,42]:
    ...
    for k, v in Folds.items():
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath=f'./logs/Seed{seed}/EEGAtlas/Fold{k} Model',
            filename=f'EEGAtlas_Fold{k}',
        ...
        trainer = pl.Trainer(accelerator='cuda',
                             ...
                             callbacks=callbacks,
                             logger=[
                                 pl_loggers.TensorBoardLogger(f'./logs/Seed{seed}/EEGAtlas/', name="EEGAtlas_tb", version=f"Fold{k}"),
                                 pl_loggers.CSVLogger(f'./logs/Seed{seed}/EEGAtlas/', name="EEGAtlas_csv", version=f"Fold{k}")])
